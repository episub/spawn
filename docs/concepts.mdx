...
Spawn is a collection of tools for building a GraphQL API witha PostgreSQL database.
...
Spawn embraces the view that the database is not an inconsequential choice, and should not be abstracted away until it is indistinguishable from other databases.  Developers and companies choose particular databases because of their virtues, and often those virtues cannot be fully realised if they are abstracted away through a generic interface that's interchangeable with other databases.
